...
def complete_cpu_performance(line):
	columns = ["host_sn", "cur_freq", "ht", "norm", "pstate", "record_id", "turbo"]
...
def complete_server_rt_status(line):
	columns = ["host_sn", "cpupower", "dimmpower", "in_temp", "out_temp", "pch_temp", "power", "power_ratio"]

	p = line.replace(":", " ").split()
	row = {'host': p[1], 'time_stamp': p[3]}
	log = get_log("server_rt_status", p, row, columns, 4)
	return log
...
if __name__ == "__main__":
	...
	cpu_basic_text.saveAsTextFile(path + "/output/cpu_failure/20141201_0100")
